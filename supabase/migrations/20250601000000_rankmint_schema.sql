-- RankMint Supabase schema
-- In SQL Editor: paste THIS ENTIRE FILE (not the file path). Then click Run.
...
-- Brand workspace (session-scoped until auth is added)
...
-- RLS: server uses service role; block anon direct access
alter table public.analysis_cache enable row level security;
alter table public.brands enable row level security;
alter table public.saved_reports enable row level security;
alter table public.shortlists enable row level security;

-- No policies for anon/authenticated — access via service role API routes only

-- Match brands by embedding (RAG retrieval)
